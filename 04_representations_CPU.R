...
aggregate_sparse_weighted_mean <- function(mat, groups, weights = NULL) {
    ...
    # tapply() returns an array-like object, force it to plain numeric
    denom <- tapply(weights, groups, sum)
    denom <- as.numeric(denom)
    names(denom) <- levels(groups)
    
    # Explicit numeric conversion prevents Matrix slot-class errors
...
build_representations <- function(dt) {
    ...
    w2v_review <- t(vapply(toks_list, doc_from_tokens, numeric(ncol(w2v_vocab)), emb = w2v_vocab))
...
", SEED, SEED, SEED, SEED, SEED))
    ...
    model <- st$SentenceTransformer("sentence-transformers/all-MiniLM-L6-v2", device = device)
    
    # Batch by hand for safety on long corpora
    batch_size <- if (device == "cuda") 128L else 32L
    starts <- seq(1L, nrow(dt), by = batch_size)
    
    sbert_review <- NULL
    for (s in starts) {
        idx <- s:min(s + batch_size - 1L, nrow(dt))
        emb <- model$encode(as.list(dt$text_raw[idx]))
        emb <- as.matrix(emb)
        sbert_review <- if (is.null(sbert_review)) emb else rbind(sbert_review, emb)
    }
